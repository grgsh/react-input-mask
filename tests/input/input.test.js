...
async function simulateBackspacePress(input) {
	...
	if (selection.length) {
		...
		input.value = value.slice(0, selection.start - 1) + value.slice(selection.end);
...
const ClassInputComponent = React.forwardRef((props, ref) => <InnerClassInputComponent innerRef={ref} {...props} />);
...
describe('react-input-mask', () => {
	...
	it('should format value on mount', async () => {
		const { input } = createInput(<Input mask="+7 (999) 999 99 99" defaultValue="74953156454" />);
		expect(input.value).to.equal('+7 (495) 315 64 54');
	});

	it('should format value with invalid characters on mount', async () => {
		const { input } = createInput(<Input mask="+7 (9a9) 999 99 99" defaultValue="749531b6454" />);
	...
	it('should handle array mask', async () => {
		...
		const { input } = createInput(<Input mask={mask} defaultValue="А 784 КТ 77" />);
	...
	it('should handle full length maskPlaceholder', async () => {
		const { input } = createInput(<Input mask="99/99/9999" maskPlaceholder="dd/mm/yyyy" defaultValue="12" />);
	...
	it('should handle escaped characters in mask', async () => {
		const { input } = createInput(<Input mask="+4\9 99 9\99 99" maskPlaceholder={null} />);
		await simulateFocus(input);
	...
	it('should handle alwaysShowMask', async () => {
		const { input, setProps } = createInput(<Input mask="+7 (999) 999 99 99" alwaysShowMask />);
		expect(input.value).to.equal('+7 (___) ___ __ __');
	...
	it('should adjust cursor position on focus', async () => {
		const { input, setProps } = createInput(<Input mask="+7 (999) 999 99 99" value="+7" />);
		await simulateFocus(input);
	...
	it('should adjust cursor position on focus on input with autoFocus', async () => {
		const { input, setProps } = createInput(<Input mask="+7 (999) 999 99 99" value="+7" autoFocus />);
		expect(getInputSelection(input).start).to.equal(4);
		expect(getInputSelection(input).end).to.equal(4);
	...
	it('should handle changes on input with autoFocus', async () => {
		const { input } = createInput(<Input mask="+7 (999) 999 99 99" autoFocus />);
	...
	it('should format value in onChange (without maskPlaceholder)', async () => {
		const { input } = createInput(<Input mask="**** **** **** ****" maskPlaceholder={null} />);
		await simulateFocus(input);
		expect(input.value).to.equal('');
	...
	it('should handle entered characters (without maskPlaceholder)', async () => {
		const { input } = createInput(
			<Input mask="+7 (999) 999 99 99" defaultValue="+7 (111) 123 45 6" maskPlaceholder={null} />
		);
		await simulateFocus(input);
	...
	it('should handle single character removal with Backspace (without maskPlaceholder)', async () => {
		const { input } = createInput(
			<Input mask="+7 (999) 999 99 99" defaultValue="74953156454" maskPlaceholder={null} />
		);
		await simulateFocus(input);
	...
	it('should adjust cursor position on single character removal with Backspace (without maskPlaceholder)', async () => {
		const { input } = createInput(
			<Input mask="+7 (999) 999 99 99" defaultValue="749531564" maskPlaceholder={null} />
		);
		await simulateFocus(input);
	...
	it('should handle single character removal with Backspace on mask with escaped characters (without maskPlaceholder)', async () => {
		const { input } = createInput(
			<Input mask="+4\9 99 9\99 99" defaultValue="+49 12 394" maskPlaceholder={null} />
		);
		await simulateFocus(input);
	...
	it('should handle mask change', async () => {
		const { input, setProps } = createInput(<Input mask="9999-9999-9999-9999" defaultValue="34781226917" />);
	...
	it('should handle mask change with on controlled input', async () => {
		const { input, setProps } = createInput(<Input mask="9999-9999-9999-9999" value="38781226917" />);
		setProps({
			onChange: () => {
				setProps({
	...
	it('should handle string paste (with maskPlaceholder)', async () => {
		const { input } = createInput(<Input mask="9999-9999-9999-9999" defaultValue="____-____-____-6543" />);
		await simulateFocus(input);
	...
	it('should handle string paste (without maskPlaceholder)', async () => {
		const { input } = createInput(
			<Input mask="9999-9999-9999-9999" defaultValue="9999-9999-9999-9999" maskPlaceholder={null} />
		);
		await simulateFocus(input);
	...
	it('should handle string paste (without maskPlaceholder) on controlled input', async () => {
		const { input, setProps } = createInput(<Input mask="9999-9999-9999-9999" maskPlaceholder={null} value="" />);
		setProps({
			value: '',
			onChange: event => {
				setProps({
	...
	it('should handle string paste at position of permanent character (with maskPlaceholder)', async () => {
		const { input } = createInput(<Input mask="9999-9999-9999" maskPlaceholder=" " />);
		await simulateFocus(input);
	...
	it('should keep placeholder on rerender on empty input with alwaysShowMask', async () => {
		const { input, setProps } = createInput(<Input mask="99-99" value="" alwaysShowMask />);
		setProps({ value: '' });
	...
	it('should show empty value when input switches from uncontrolled to controlled', async () => {
		const { input, setProps } = createInput(<Input mask="+7 (*a9) 999 99 99" />);
	...
	it("shouldn't affect value if mask is empty", async () => {
		...
		const { input, setProps } = createInput(<Input value="12345" onChange={() => {}} />);
	...
	it('should show next permanent character when maskPlaceholder is null', async () => {
		const { input } = createInput(<Input mask="99/99/9999" value="01" maskPlaceholder={null} />);
	...
	it('should show all next consecutive permanent characters when maskPlaceholder is null', async () => {
		const { input } = createInput(<Input mask="99---99" value="01" maskPlaceholder={null} />);
		expect(input.value).to.equal('01---');
	});

	it('should show trailing permanent character when maskPlaceholder is null', async () => {
		const { input } = createInput(<Input mask="99%" value="10" maskPlaceholder={null} />);
	...
	it('should allow to modify value with beforeMaskedStateChange', async () => {
		...
		const { input, setProps } = createInput(
			<Input mask="99/99/9999" value="" beforeMaskedStateChange={beforeMaskedStateChange} />
		);
		expect(input.value).to.equal('');
	...
	it('should handle autofill', async () => {
		const { input } = createInput(<Input mask="9999-9999" defaultValue="123" maskPlaceholder={null} />);
		await simulateFocus(input);
	...
	it('should handle transition between masked and non-masked state', async () => {
		// no-op onChange to prevent test warning
		const { input, setProps } = createInput(<Input value="" onChange={() => {}} />);
		setProps({
			value: '',
			onChange: event => {
				setProps({
	...
	it('should handle change event without focus', async () => {
		const { input } = createInput(<Input mask="+7 (999) 999 99 99" maskPlaceholder={null} />);
