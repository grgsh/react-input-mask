...
export default class MaskUtils {
	...
	isPositionEditable = (position: number): boolean => {
		...
		return !!(mask && position < mask.length && permanents.indexOf(position) === -1);
	};
	...
	isValueEmpty = (value: string): boolean =>
		value
			...
			.every(
				(character, position) =>
					!this.isPositionEditable(position) || !this.isCharacterFillingPosition(character, position)
			);

	isValueFilled = (value: string): boolean => {
		const { lastEditablePosition } = this.maskOptions;
		return lastEditablePosition !== null && this.getFilledLength(value) === lastEditablePosition + 1;
	};
	...
	getStringFillingLengthAtPosition = (string: string, position: number): number => {
		...
		const insertedValue = characters.reduce(
			(value, character) => this.insertCharacterAtPosition(value, character, value.length),
			repeat(' ', position)
		);
	...
	insertCharacterAtPosition = (value: string, character: string, position: number): string => {
		...
		const isNextPlaceholder =
			maskPlaceholder && nextEditablePosition ? character === maskPlaceholder[nextEditablePosition] : null;
		const valueBefore = value.slice(0, position);

		if (isAllowed || !isEditable) {
			const insertedCharacter = isAllowed ? character : (mask[position] as string);
	...
	insertStringAtPosition = (value: string, string: string, position: number): string => {
		...
		if (isFixedLength) {
			...
			value = editableCharactersAfter.reduce((value, character) => {
				const nextEditablePosition = this.getRightEditablePosition(value.length);
	...
	processChange = (currentState: InputState, previousState: InputState): ProcessChangeResult => {
		...
		if (selection.end > previousSelection.start) {
			...
			formattedEnteredStringLength = this.getStringFillingLengthAtPosition(enteredString, cursorPosition);
		...
		newValue = this.insertStringAtPosition(newValue, enteredString, cursorPosition);
		...
		if (cursorPosition >= mask.length) {
			...
			cursorPosition = this.getRightEditablePosition(cursorPosition) || cursorPosition;
		}
